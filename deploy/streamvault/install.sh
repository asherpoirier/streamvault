...
check_source_files() {
    # Check if source files exist in the same directory as the script
    if [[ ! -d "$SCRIPT_DIR/backend" ]] || [[ ! -d "$SCRIPT_DIR/frontend" ]]; then
        log_error "Source files not found!"
        log_error "Expected 'backend' and 'frontend' directories in: $SCRIPT_DIR"
        log_error ""
        log_error "Make sure you extracted the full streamvault package."
        log_error "Directory structure should be:"
        log_error "  streamvault/"
        log_error "  ├── install.sh (this script)"
        log_error "  ├── uninstall.sh"
        log_error "  ├── backend/"
        log_error "  └── frontend/"
        exit 1
    fi
...
install_mongodb() {
    ...
    # Add MongoDB repository (use jammy for 24.04 as noble may not be available yet)
...
setup_application() {
    ...
    # Copy application files from the package directory
