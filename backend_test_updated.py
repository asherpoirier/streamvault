...
class StreamVaultAPITester:
    def __init__(self, base_url="https://mediastream-403.preview.emergentagent.com"):
        ...
        self.tests_passed = 0
        self.test_results = []
        self.admin_username = "admin_010140"  # Use existing admin
        self.admin_password = "TestPass123!"  # Assuming same password pattern
    ...
    def test_register_new_user(self):
        """Test registering a new user (should not be admin)"""
        try:
            ...
            if response.status_code == 200:
                ...
                # Should NOT be admin since there's already an admin
                success = not is_admin
                self.log_test("Register New User (Non-Admin)", success, 
                            f"Admin: {is_admin} (should be False)")
                return success
            else:
                self.log_test("Register New User (Non-Admin)", False, 
                            f"Status: {response.status_code}, Response: {response.text}")
                return False
        except Exception as e:
            self.log_test("Register New User (Non-Admin)", False, str(e))
            return False
